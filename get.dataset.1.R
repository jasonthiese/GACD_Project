# function to get a dataset (for training or testing data)
# param string dataset.type: the type of dataset ("train" or "test")
# returns a dataframe containing the Subject, Activity, and all mean or std features
get.dataset.1 <- function(dataset.type) {  
    ...
    get.feature.info <- function(file.path = "data/UCI HAR Dataset/features.txt", name.column = 2) {
        ...
        # get the indices of the elements containing "mean()" or "std()"
        indices <- integer()
        for (i in 1:length(features)) {
            if ((regexpr("mean\\()", features[i]) != -1) || (regexpr("std()", features[i]) != -1)) {
                indices <- c(indices, i)
            }
        }
        
        # get the names corresponding to those indices
        names <- features[indices]
        
        # return the two vectors in a list
        return (list(indices=indices, names=names))
    }
    
    dataset <- cbind(get.subject(dataset.type), get.activity(dataset.type), get.features(dataset.type))
    return(cbind(get.use(dataset.type, nrow(dataset)), dataset))
}
